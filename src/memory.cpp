...
#ifdef WINTER_USE_MMAP
    if (_internal.start)
        munmap(_internal.start, _internal.current_capacity_pages.get() << WASM_PAGE_SHIFT);
#else
    std::free(_internal.start);
#endif
}

bool Memory::alloc_exactly(NumPages num_pages) {
    WASSERT(!is_shared(), "Shared WebAssembly memory cannot be grown");
    WASSERT(num_pages >= _internal.current_capacity_pages, "WebAssembly memory cannot be shrunk");
    WASSERT(num_pages <= _internal.max_capacity_pages, "WebAssembly memory cannot grow beyond its max capacity");

    if (num_pages != _internal.current_capacity_pages && num_pages != NumPages(0)) {
...
#ifdef WINTER_USE_MMAP
        void* new_start = _internal.start
            ? mremap(_internal.start, _internal.current_capacity_pages.get() << WASM_PAGE_SHIFT, new_size, MREMAP_MAYMOVE)
            : mmap(nullptr, new_size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);

        if (new_start == MAP_FAILED)
            return false;
#else
        void* new_start = std::realloc(_internal.start, new_size);

        if (!new_start)
            return false;

        std::memset(
            static_cast<void*>(static_cast<uint8_t*>(new_start) + (_internal.current_capacity_pages.get() << WASM_PAGE_SHIFT)),
            0,
            (num_pages - _internal.current_capacity_pages).get() << WASM_PAGE_SHIFT
        );
#endif

        _internal.start = new_start;
        _internal.current_capacity_pages = NumPages(new_size >> WASM_PAGE_SHIFT);
    }
...
bool Memory::alloc_at_least(NumPages num_pages) {
    if (num_pages <= _internal.current_capacity_pages)
        return true;

    if (num_pages > _internal.max_capacity_pages)
        return false;
...
NumPages Memory::grow(NumPages new_pages) {
    ...
    if (is_shared()) {
        ...
        _internal.size = new_size_pages.get() << WASM_PAGE_SHIFT;
        return old_size_pages;
    }
}
